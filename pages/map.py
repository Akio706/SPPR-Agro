...
def map_page(action: str = None, fields: str = None, field_id: str = None):
    ...
    with ui.column().classes('items-center justify-center min-h-screen bg-grey-2'):
        with ui.card().classes('w-full max-w-4xl shadow-lg mt-8'):
            ui.label('Карта и список полей').classes('text-h4 q-mb-md text-center')
        ...
        # Обработка редактирования полигона (оставляем как было)
        ...
        ui.button('Назад', on_click=lambda: ui.open('/')).classes('mt-4') 